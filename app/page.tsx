...
export default function Home() {
  ...
  function performExport() {
    ...
    setTimeout(() => {
      ...
      const header = view === "overview" ? ["游戏", "用户名称", "用户ID", "区域", "活跃用户", "游戏次数", "用户投入", "用户出奖", "盈亏", "返奖率", "热度"] : ["游戏排行", "用户ID", "昵称", "区域", "游戏", "活跃天数", "游戏次数", "用户投入", "用户出奖", "盈亏", "返奖率", "最近游戏时间"];
      const exportRows = view === "overview"
        ? (rows as GameRow[]).map((row) => { const user = summaryUserForGame(row); return [row.game, user.nickname, user.id, row.region, row.active, row.plays, row.input, row.output, row.net, row.rate, row.rank]; })
