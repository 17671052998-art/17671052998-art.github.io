...
function summaryUserForGame(row: GameRow) {
  return users.find((user) => user.game === row.game && user.region === row.region)
    ?? users.find((user) => user.region === row.region)
    ?? users.find((user) => user.game === row.game)
    ?? users[0];
}

function SummaryUserCell({ user }: { user: UserRow }) {
  return (
    <div className="summary-user-cell">
      <span className="summary-user-avatar" aria-hidden="true">{user.nickname.slice(0, 1).toUpperCase()}</span>
      <span className="summary-user-copy"><strong>{user.nickname}</strong><small>ID {user.id}</small></span>
    </div>
  );
}
...
export default function Home() {
  ...
  function performExport() {
    ...
    setTimeout(() => {
      ...
      const header = view === "overview" ? ["用户名称", "用户ID", "游戏", "区域", "活跃用户", "游戏次数", "用户投入", "用户出奖", "盈亏", "返奖率", "热度"] : ["游戏排行", "用户ID", "昵称", "区域", "游戏", "活跃天数", "游戏次数", "用户投入", "用户出奖", "盈亏", "返奖率", "最近游戏时间"];
      const exportRows = view === "overview"
        ? (rows as GameRow[]).map((row) => { const user = summaryUserForGame(row); return [user.nickname, user.id, row.game, row.region, row.active, row.plays, row.input, row.output, row.net, row.rate, row.rank]; })
