          ...
          {view === "overview" ? (
            <>
              ...
              <section className="analytics-row">
                <article className="panel regional-statistics-panel"><div className="panel-title"><h2>游戏区域资金统计</h2><span>{gameFilterVendor(game) ? `按区域汇总${gameFilterLabel(game)}的用户投入、用户出奖与盈亏` : `${game} 各区域用户投入、用户出奖与盈亏`}</span></div><div className="region-stat-head" aria-hidden="true"><span>排名</span><span>区域</span><span>游戏</span><span>用户投入</span><span>用户出奖</span><span>盈亏</span></div><div className="region-stat-list">{regionInvestmentStats.length ? regionInvestmentStats.map((item, index) => <div className="region-stat-row" key={item.region}><b className={index === 0 ? "first" : ""}>{index + 1}</b><strong>{item.region}</strong><span>{gameFilterLabel(game)}</span><em title={`${moneyText(item.input)} 金币`}>{money(item.input)}</em><em title={`${moneyText(item.output)} 金币`}>{money(item.output)}</em><em title={`${item.input - item.output < 0 ? "亏损 " : "盈利 "}${moneyText(Math.abs(item.input - item.output))} 金币`}>{profitLoss(item.input - item.output)}</em></div>) : <div className="region-stat-empty">暂无匹配区域数据</div>}</div></article>
              </section>
